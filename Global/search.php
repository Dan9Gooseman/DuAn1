...
if (!empty($tukhoa)) {
    $sql = "select sp.sp_id, sp.sp_ten, MIN(btsp.btsp_giatien) AS min_giatien, MAX(btsp.btsp_giatien) AS max_giatien
                    FROM sanpham sp
                    ...
                    WHERE sp.sp_ten like '%$tukhoa%' and sp.is_active = 1 AND dm.is_active = 1 AND th.is_active = 1 AND dt.is_active = 1
                    GROUP BY
                    sp.sp_id, sp.sp_ten";
