...
function damuahangchua($sp_id)
{
    if (isset($_SESSION['nguoidung'])) {
        ...
        $sql = "select sp.sp_id
                FROM user u 
                JOIN donhang dh ON u.user_id = dh.user_id 
                JOIN donhangchitiet dhct ON dhct.dh_id = dh.dh_id
                JOIN bienthesanpham btsp on dhct.btsp_id = btsp.btsp_id
                JOIN sanpham sp on btsp.sp_id = sp.sp_id 
                WHERE u.user_id = '$user_id' and dh.dh_trangthaidonhang = 'Giao Hàng Thành Công' and sp.sp_id ='$sp_id' ";
        $damuahangchua = pdo_query_one($sql);
        if ($damuahangchua != false) {
            $damua = 'roi';
        } else {
            $damua = 'chua';
        }
